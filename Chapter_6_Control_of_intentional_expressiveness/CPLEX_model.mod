...
execute {
    // Open/create CSV
    var csvFile = new IloOplOutputFile("CPLEX_output.csv");

    // CSV header
    csvFile.writeln("note,string,fret,finger,handposition");

    for (i in Notes) {
        // Open strings
        for (s in Strings) {
            if (o[i][s] == 1) {
                csvFile.writeln(note[i] + "," + s + ",0,0,0"); // Open string, fret and finger are 0, handposition is 0
            }
        }
        ...
        for (p in Frets) {
            for (h in Fingers) {
                for (c in AllCombinations) {
                    if (y[i][p][h][c] == 1) {
                        var foundHandPosition = 0; // Default to 0 if no hand position is found
                        for (hp in HandPosition) {
                            if (x[i][hp] == 1) {
                                foundHandPosition = hp; // Set the hand position
                                break;
                            }
                        }
                        csvFile.writeln(note[i] + "," + c.corda + "," + c.fret + "," + h + "," + foundHandPosition);
                    }
                }
            }
        }
